...
function fetchUser(userAddress?: string): Promise<{ id: number; name: string }> {
...
const user = vueRef<{ id: number; username?: string; name: string } | null>(null)
...
async function persistUsersToFile() {
  ...
  const { fileURLToPath } = await import('url')
    const filePath = fileURLToPath(new URL('../models/tempUser.json', import.meta.url))
...
export async function login(username: string, password: string) {
  ...
  const found = userList.find((u) => u.userid === username && u.password === password)
  if (found) {
...
export async function updateUser(username: string, data: { name?: string; password?: string }) {
...
export async function createUser(userObj: { username: string; password: string; name: string }) {
  ...
  userList.push({ userid: userObj.username, password: userObj.password, username: userObj.name })
  const persisted = await persistUsersToFile()
  return { success: true, persisted }
}
